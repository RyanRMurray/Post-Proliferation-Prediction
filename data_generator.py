...
class TrainingData:
    def __init__(self, t_data, u_data, truth):
        training_num = 0.8
        self.t_train, self.u_train, self.truth_train = (
            np.empty(shape=(1, SEQ_LENGTH), dtype="uint32"),
            np.zeros(shape=(1, DETAIL_FEATURES), dtype="uint32"),
            np.empty(shape=(1, 1)),
        )
        self.t_valid, self.u_valid, self.truth_valid = (
            np.empty(shape=(1, SEQ_LENGTH), dtype="uint32"),
            np.zeros(shape=(1, DETAIL_FEATURES), dtype="uint32"),
            np.empty(shape=(1, 1)),
        )
        ...
        for i in range(CATEGORIES):
            ...
            if len(t_v) != 0:
                ...
                if len(t_t) > 0:
                    self.truth_train = np.concatenate(
                        (self.truth_train, np.array([[i]] * len(t_t)))
                    )
                self.truth_valid = np.concatenate(
                    (self.truth_valid, np.array([[i]] * len(t_v)))
                )
        ...
        self.truth_train = keras.utils.np_utils.to_categorical(
            self.truth_train, CATEGORIES
        )
        self.truth_valid = keras.utils.np_utils.to_categorical(
            self.truth_valid, CATEGORIES
        )
        ...
        map(
            ...
            [
                ...
                (
                    datetime.now()
                    - datetime.strptime(
                        tweet["author_data"]["created_at"], "%Y-%m-%dT%H:%M:%S.000Z"
                    )
                ).days
                / 30,
...
def generate_training_data_old(data, image_directory, text_input_size, tokenizer=None):
    ...
    random.shuffle(data)
    for tweet in data:
        ((_, t, u), tr) = tweet_to_training_pair(
            tweet, image_directory, text_input_size
        )
