...
parser = argparse.ArgumentParser(
    description="Generate and train a model to predict success of Twitter posts"
)
parser.add_argument(
    "dataset", metavar="dataset", type=str, help="Path to the training data set"
)
parser.add_argument(
    "glove", metavar="glove", type=str, help="Path to the training GLoVE training set"
)
parser.add_argument(
    "tokenizer",
    metavar="tokenizer",
    type=str,
    help="Path to a tokenizer with associated metrics",
)
parser.add_argument(
    "modeltype",
    metavar="modeltype",
    ...
    help="Version of model to build",
)
parser.add_argument(
    "--use-balanced",
    dest="balanced",
...
def model_new(matrix, modeltype: ModelType):

    if modeltype != ModelType.Details:
        # text layer
        t_input = Input(shape=(SEQ_LENGTH,))
        t_branch = Embedding(
            100_000, 100, input_length=SEQ_LENGTH, weights=[matrix], trainable=False
        )(t_input)
    ...
    if modeltype == ModelType.Details:
        ...
        final = tf.keras.Model(inputs=t_input, outputs=model)
    else:
...
def main():
    ...
    if args["balanced"]:
        weights = {0: 1, 1: 2, 2: 2, 3: 17, 4: 408, 5: 6667}
        batch_size = 32
    else:
        weights = {0: 1, 1: 24, 2: 151, 3: 1231, 4: 17910, 5: 203000}
        batch_size = 500
    ...
    model: tf.keras.Model = model_new(matrix, args["modeltype"])
    ...
    if args["modeltype"] == ModelType.Details:
        x_train = formatted.x_train()[1]
        x_valid = formatted.x_valid()[1]
    elif args["modeltype"] == ModelType.Text:
        x_train = formatted.x_train()[0]
        x_valid = formatted.x_valid()[0]
    else:
        x_train = formatted.x_train()
        x_valid = formatted.x_valid()

    h = model.fit(
        x=x_train,
        y=formatted.y_train(),
        validation_data=(x_valid, formatted.y_valid()),
        epochs=EPOCHS,
...
def create_tokenizer_old(data, max_words=None):
    ...
    if max_words is None:
        print(
            "{} unique symbols, input size is {}. Using {} dimensions.".format(
                len(words), input_size, dims
            )
        )
        tokenizer = Tokenizer(num_words=len(words))
    else:
        print(
            "{} unique symbols, truncating to {}, input size is {}. Using {} dimensions.".format(
                len(words), max_words, input_size, dims
            )
        )
...
def main1():
    ...
    formatted_data = generate_training_data(
        data, args["imageset"], text_input_length, tokenizer
    )
    ...
    if args["model"] is None:
        ...
        model: tf.keras.Model = build_model_test(
            name, formatted_data, word_count, text_input_length, dims
        )
    ...
    """
    model.compile(
        optimizer=Adamax(learning_rate=0.0001, clipvalue=5),
        metrics=["accuracy"],
        loss="categorical_crossentropy",
    )
    model.summary()
    ...
    save_every = (
        len(data) // 1000
    ) * 10  # unsimplified for clarity: save every 100 epochs
    ...
    h = model.fit(
        ...
        validation_data=(
            [formatted_data.x_valid()[1], formatted_data.x_valid()[2]],
            formatted_data.y_valid(),
        ),
        epochs=500,
        # batch_size=1000,
        verbose=1,
